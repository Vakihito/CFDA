...
class ClassificationModel(nn.Module):
    ...
    def forward(self, cluster_number, inputs_list=[]):
        output_list = []
        cluster_number = torch.unsqueeze(cluster_number, dim=-1)
        attention_weights = self.cluster_attention(cluster_number)

        for i in range(n_clusters):
          cur_att_weight = attention_weights[:,i].unsqueeze(1)
          output_list.append((inputs_list[:,i] * cur_att_weight) + inputs_list[:,i])
...
df_test[['index', 'prediction', "pred_indexes"]].to_pickle(f'{save_path}/cluster_fusion_predictions_cluster_att.pkl')
